...
{
    public class AraneaeHoldout : ModProjectile
    {
        public override bool? CanDamage() => false;
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Gun Holdout");
        ...
        public override void AI()
        {
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                if (player.channel && !player.noItems && !player.CCed && player.GetModPlayer<MalignantPlayer>().BuildCount < 6)
                {
                    if (Projectile.ai[1]++ % 20 == 0 && EnergySpawnCounter < 6)
                    {
                        ...
                        {
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), RandomPosition, Vector2.Zero, ModContent.ProjectileType<NjorEnergy>(), 0, 0f, Main.myPlayer, player.whoAmI);
                        }
                ...
                else
                {
                    ...
                    if (player.GetModPlayer<MalignantPlayer>().BuildCount == 6)
                    {

                        if (Projectile.ai[1] % 5 == 0)
                        {
                            SoundEngine.PlaySound(SoundID.DD2_PhantomPhoenixShot, Projectile.Center);
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, ProjectileVelocity * 4, ModContent.ProjectileType<Fangs>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                        }
                    }
                    else
                    {
                    ...
                    {
                        player.GetModPlayer<MalignantPlayer>().BuildCount = 0;
                        Projectile.Kill();
                    }
                }
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            SpriteEffects effects = Main.player[Projectile.owner].direction == 2 ? SpriteEffects.FlipHorizontally : SpriteEffects.None;
