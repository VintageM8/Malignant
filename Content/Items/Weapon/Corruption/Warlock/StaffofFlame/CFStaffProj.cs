...
namespace Malignant.Content.Items.Weapon.Corruption.Warlock.StaffofFlame
{
    public class CFStaffProj : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Cursed Fire that HEHEHAs");
        }
        ...
        public override void AI()
        {
            ...
            {
                int num1110 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.CursedTorch, Projectile.velocity.X, Projectile.velocity.Y, 50, default(Color), 1.5f);
                Main.dust[num1110].position = (Main.dust[num1110].position + Projectile.Center) / 2f;
                Main.dust[num1110].noGravity = true;
                Dust dust81 = Main.dust[num1110];
                dust81.velocity *= 0.5f;

                Dust dustTrail = Dust.NewDustPerfect(new Vector2(Projectile.position.X, Projectile.position.Y), DustID.CorruptTorch, Projectile.velocity);
