...
{
    public class CorruptedCordProj : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int i = 0; i < i++;)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), player.Center.X, player.Center.Y, (float)(Math.Sin(offsetAngle) * 3f), (float)(Math.Cos(offsetAngle) * 3f), ModContent.ProjectileType<CursedWave>(), damage, 0, player.whoAmI);
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), player.Center.X, player.Center.Y, (float)(-Math.Sin(offsetAngle) * 3f), (float)(-Math.Cos(offsetAngle) * 3f), ModContent.ProjectileType<CursedWave>(), damage, 0, player.whoAmI);
