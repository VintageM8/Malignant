...
namespace Malignant.Content.Items.Weapon.Crimson.FleshBlazer
{
    public class FleshBlazer : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (FlameCount >= 4)
            {
                Terraria.Audio.SoundEngine.PlaySound(SoundID.DD2_BetsyFireballShot, player.position);
                Projectile.NewProjectile(source, position, velocity, ModContent.ProjectileType<BlazerBomb>(), damage, knockback, player.whoAmI);
                FlameCount = 0;
            }
            return false;
        }
