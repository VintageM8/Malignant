...
{
    class ArterionBossBag : ModItem
    {
        public override int BossBagNPC => NPCType<Arterion>();
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Treasure Bag (Arterion)");
        ...
        public override void OpenBossBag(Player Player)
        {
            ...
            for (int k = 0; k < (Main.masterMode ? 3 : 2); k++)
            {
                switch (weapon % 2)
                {
                    case 0: Player.QuickSpawnItem(Player.GetSource_OpenItem(Item.type), ItemType<HerzanfallDagger>()); break;
                    case 1: Player.QuickSpawnItem(Player.GetSource_OpenItem(Item.type), ItemType<BurstingArtery>()); break;
                }

                weapon++;
            }
            Player.QuickSpawnItem(Player.GetSource_OpenItem(Item.type), ItemType<HeartMoniter>());
        }
        ...
        public override void PostUpdate()
        {
            ...
            if (Item.timeSinceItemSpawned % 12 == 0)
            {
                ...
                var dust = Dust.NewDustPerfect(center + direction * distance, DustID.Blood, velocity);
                dust.scale = 0.5f;
                dust.fadeIn = 1.1f;
                dust.noGravity = true;
                dust.noLight = true;
