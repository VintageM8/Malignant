using Malignant.Content.Items.Accessories.Expert.Lich;
...
namespace Malignant.Content.Items.Misc.Bags
{
    public class WarlockBossBag : ModItem
    {
        ...
        public override void OpenBossBag(Player player)
        {
            ...
            if (choice == 0) // 
            {
                player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemType<NightsisterBlade>(), 1);
            }
            else if (choice == 1)
            {
                player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemType<CursedFireballStaff>(), 1);
            }
            else if (choice == 2)
            {
                player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemType<Boeyr>(), 1);
            }
            else if (choice == 2)
            {
                player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemType<CorruptedCord>(), 1);
            }
