...
{
    public class LoyalLensSummon : ModProjectile
    {
        int timer = 0;
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                if (timer >= Main.rand.Next(50, 90))
                {
                    ...
                    {
                        if (colorType == 0)
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center.X, Projectile.Center.Y, direction.X, direction.Y, ModContent.ProjectileType<Gernade1>(), 0, Main.myPlayer);
                        else
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center.X, Projectile.Center.Y, direction.X, direction.Y, ModContent.ProjectileType<Gernade1>(), 0, Main.myPlayer);
                    }
                }
