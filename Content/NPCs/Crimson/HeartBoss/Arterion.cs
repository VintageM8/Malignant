                ...
                if (AITimer >= 50)
                {
                    ...
                    if (AITimer == 75 || AITimer == 125 || AITimer == 175 || AITimer == 225 || AITimer == 275)
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            double offsetAngle = (startAngle + deltaAngle * (i + i * i) / 2f) + 32f * i;
                            int damage = expertMode ? 32 : 48;
                            Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y, (float)(Math.Sin(offsetAngle) * 3f), (float)(Math.Cos(offsetAngle) * 3f), ModContent.ProjectileType<LeechingBlast>(), damage, 0, player.whoAmI);
                            Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y, (float)(-Math.Sin(offsetAngle) * 3f), (float)(-Math.Cos(offsetAngle) * 3f), ModContent.ProjectileType<LeechingBlast>(), damage, 0, player.whoAmI);
                        }
                    }
                }
