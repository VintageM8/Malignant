...
namespace  Malignant.Content.NPCs.Crimson.Heart
{
    public class BloodBlister : ModProjectile
    {
        ...
        {
            for (int i = 0; i < 2; i++)
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, new Vector2(-Projectile.velocity.X + Main.rand.Next(-2, 2), -Projectile.velocity.Y + Main.rand.Next(-2, 2)), ModContent.ProjectileType<BloodSpurt>(), Projectile.damage / 2, Projectile.knockBack, Projectile.owner, 0, 1);
