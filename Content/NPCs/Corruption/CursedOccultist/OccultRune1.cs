...
{
    public class OccultRune1 : ModProjectile
    {
            ...
            else
            {
                ...
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    if (distance <= 400 && !Main.npc[i].friendly && Projectile.ai[0] >= 90)
                    {
                        ...
                        {
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.One.RotatedByRandom(Math.PI) * 4, ProjectileType<Rune_1>(), Projectile.damage, 3f, Main.myPlayer);
